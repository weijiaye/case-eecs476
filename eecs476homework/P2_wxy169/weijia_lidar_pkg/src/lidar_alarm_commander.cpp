...
void laserCallback(const sensor_msgs::LaserScan& laser_scan) 
{
      ...
      for (float y=-1.57;y<1.57;y=y+0.1) 
{ //each time y will increase 0.1;
          ping_index_ = (int) ((y -angle_min_)/angle_increment_);
       ...
       if (y<0.5&&y>-0.5) 
{
          if (ping_dist_in_front_<0.4/sin(y+1.57)) 
         {
            ROS_WARN("DANGER, YOU WILL BE DIE!");
       ...
       else if (ping_dist_in_front_<0.3/cos(1.57-fabs(y))) 
{
             float m=0.3/cos(1.57-abs(y));
             ROS_INFO("m=%f",m);
             ROS_INFO("y=%f",y);
             ROS_INFO("absy=%f",fabs(y));
             ROS_WARN("danger, you are stupid!");
             laser_alarm_=true;break;
           }
             else laser_alarm_=false;
          }
