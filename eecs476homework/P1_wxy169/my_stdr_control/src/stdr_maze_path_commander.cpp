...
int main(int argc, char **argv) {
    ...
    // start with all zeros in the command message; should be the case by default, but just to be safe..ros
    ...
    //halt the motion
    twist_cmd.linear.x=0.0; //and stop move forward
    twist_cmd.angular.z=-yaw_rate;//and start spining in place
    timer=0.0;//resent the timer 
    while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }
     twist_cmd.angular.z=0.0;//and stop spining in place
     twist_cmd.linear.x=1.5*speed;//and move forward again
     timer=0.0;//reset the timer
     while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
      ...
      twist_cmd.linear.x=0.0;//and stop move forward 
      twist_cmd.angular.z=yaw_rate;//and spining in place
      timer=0.0;//reset the timer
      while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
      ...
      twist_cmd.angular.z=0.0;//and stop spining in place
      twist_cmd.linear.x=0.5*speed;//and start move forward again
      timer=0.0;//reset the timer
      while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
      ...
      while(timer<time_3_sec) {
       ...
       twist_cmd.angular.z=0.0;//and stop spining in place
       twist_cmd.linear.x=1.6*speed;//and start move forward again
       timer=0.0;//reset the timer
       while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
       ...
       twist_cmd.linear.x=0.0;//and stop move forward
       twist_cmd.angular.z=1.1*yaw_rate;//and start spining in place
       timer=0.0;//reset the timer
       while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
        ...
        twist_cmd.angular.z=0.0;//and stop spining in place
        twist_cmd.linear.x=0.4*speed;//and start move forward
        timer=0.0;//reset the timer
        while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
        ...
        twist_cmd.linear.x=0.0;//and stop move forward
        twist_cmd.angular.z=1.1*yaw_rate;//and spining in place
        timer=0.0;//reset the timer
        while(timer<time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
