...
void DesStatePublisher::pub_next_state() { 
    ...
    if (e_stop_reset_) {
        e_stop_reset_ = false; 
        e_stop_stop=false;//reset trigger
    ...
    switch (motion_mode_) {
        ...
        //this state must be reset by a service
            ...
            if((g_lidar_alarm==false)&&(e_stop_stop==false)) motion_mode_=PURSUING_SUBGOAL;
            break;
        ...
        ROS_INFO("PURSUING_SUBGOAL");//if have remaining pts in computed traj, send them
        ...
        case DONE_W_SUBGOAL:
        ROS_INFO("DONE_W_SUBGOAL"); //suspended, pending a new subgoal
